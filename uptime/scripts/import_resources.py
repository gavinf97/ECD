...
Re-running is safe: for ids already in resources.yml, hand edits to `enabled` and
`check` are preserved. Manual entries override spreadsheet rows with the same id.
...
def write_yaml(resources: list[dict], out: Path, xlsx: Path, division: str) -> None:
    header = (
        ...
        "# Safe to hand-edit `enabled` and `check` (preserved on re-import).\n"
        "# check.url overrides the checked URL (use when the listed URL has moved).\n"
